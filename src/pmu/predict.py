...
from . import dataset, db, evaluate as ev, features as ft
from .train import Decoupage, ModelePmu
...
def entrainer(conn, *, avec_marche: bool = False, jusqua: date | None = None) -> dict:
    ...
    modele = ModelePmu(cible="y_gagnant", avec_marche=avec_marche).entrainer(df, decoupage)
    ...
    log.info("modèle %s entraîné\n%s", nom, ev.afficher(rapport))

    (DOSSIER_MODELES / nom).mkdir(parents=True, exist_ok=True)
    (DOSSIER_MODELES / nom / "rapport.txt").write_text(
        ev.afficher(rapport), encoding="utf-8"
    )
    return rapport
...
def pronostiquer(conn, jour: date | None = None, *, modeles=("sans_marche",)) -> pd.DataFrame:
    ...
    df = ft.construire(brut, avec_marche=True)
    du_jour = df[df["date_reunion"] == jour].copy()
    ...
    for nom in modeles:
        ...
        if not (chemin / "modele.joblib").exists():
            log.warning("modèle %s absent (%s) — ignoré", nom, chemin)
            continue
        modele = ModelePmu.charger(chemin)
    ...
    lignes = [
        (int(r.course_id), int(r.num_pmu), float(r.proba), int(r.rang),
         ...
         None if pd.isna(r.cote) else float(r.cote), r.modele)
    ...
    conn.cursor().executemany(
        ...
        ON CONFLICT (course_id, num_pmu, modele) DO UPDATE SET
            ...
            cote = EXCLUDED.cote, calcule_le = now()
        """,
        lignes,
    )
    conn.commit()
...
def lire_pronostics(conn, jour: date, modele: str = "sans_marche") -> list[dict]:
    ...
    rows = conn.execute(
        ...
        SELECT
            ...
            pr.calcule_le,
            ch.nom AS cheval, pd.nom_affiche AS driver, p.musique,
